...
def test_refresh_watchlist_prices_continues_after_failed_ticker(monkeypatch):
    ...
    monkeypatch.setattr(
        watchlist_service,
        "get_watchlist_items",
        lambda db: [{"secid": "SBER"}, {"secid": "BROKEN"}],
    )
    ...
    result = watchlist_service.refresh_watchlist_prices(db)
...
def test_check_active_price_alerts_continues_after_failed_alert(monkeypatch):
    ...
    monkeypatch.setattr(
        alerts_service,
        "get_active_alerts",
        lambda db: [SimpleNamespace(id=1), SimpleNamespace(id=2)],
    )
    ...
    result = alerts_service.check_active_price_alerts(db)
